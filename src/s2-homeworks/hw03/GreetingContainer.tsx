...
export const pureAddUser = (name: string, setError: (error: string) => void, setName: (name: string) => void, addUserCallback: (v: string) => void) => {
    !name ? setError("Too short") : addUserCallback(name)
    setName('')
...
export const pureOnBlur = (name: string, setError: (error: string) => void) => { // если имя пустое - показать ошибку
    ...
    name.trim() === '' ? setError("Too short") : ''
}

export const pureOnEnter = (e: KeyboardEvent<HTMLInputElement>, addUser: (value: string) => void) => { // если нажата кнопка Enter - добавить
    if (e.key === 'Enter') {
